...
function resetExpenseForm() { var form = expenseControl('expenseForm'); if (form) form.reset(); expenseControl('expenseEditId').value = ''; expenseControl('expenseDate').value = new Date().toISOString().split('T')[0]; expenseControl('expenseFormTitle').textContent = 'ثبت هزینه جدید'; }
function openExpenseForm(id) { resetExpenseForm(); if (!id) return; getBridge().get_expenses('').then(function(expenses) { var expense = expenses.find(function(item) { return item.id === id; }); if (!expense) return; expenseControl('expenseEditId').value = expense.id; expenseControl('expenseTitle').value = expense.title || ''; expenseControl('expenseAmount').value = expense.amount || ''; expenseControl('expenseDate').value = expense.expense_date || ''; expenseControl('expenseMethod').value = expense.payment_method || 'نقدی'; expenseControl('expenseNotes').value = expense.notes || ''; expenseControl('expenseFormTitle').textContent = 'ویرایش هزینه'; loadExpenseCategories(expense.category_id); }); }
function saveExpense() { var id = expenseControl('expenseEditId').value; var data = { title: expenseControl('expenseTitle').value.trim(), category_id: expenseControl('expenseCategory').value || null, amount: parseFloat(expenseControl('expenseAmount').value), expense_date: expenseControl('expenseDate').value, payment_method: expenseControl('expenseMethod').value, notes: expenseControl('expenseNotes').value }; if (!data.title || !data.amount) { showToast('خطا', 'عنوان و مبلغ هزینه الزامی است', 'error'); return; } var call = id ? getBridge().update_expense(parseInt(id), data) : getBridge().add_expense(data); call.then(function(result) { if (!result.success) throw new Error(result.message || 'ذخیره انجام نشد'); resetExpenseForm(); renderExpenses(); updateDashboard(); showToast('موفق', id ? 'هزینه ویرایش شد' : 'هزینه ثبت شد'); })['catch'](function(error) { showToast('خطا', error.message, 'error'); }); }
